  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "        self.encoder_att = nn.Linear(feature_dim, attention_dim)\n",
        ...
        "        att1 = self.encoder_att(encoder_outputs)  # (batch_size, seq_len, attention_dim)\n",
        "        att2 = self.decoder_att(decoder_hidden).unsqueeze(1)  # (batch_size, 1, attention_dim)\n",
        "        att = self.relu(att1 + att2)  # (batch_size, seq_len, attention_dim)\n",
        "\n",
        "        batch_size, seq_len, att_dim = att.shape\n",
        "        att = att.view(batch_size * seq_len, att_dim)  # flatten\n",
        "        att = self.full_att(att)  # (batch_size * seq_len, 1)\n",
        "        att = att.view(batch_size, seq_len)  # reshape back\n",
        "\n",
        "        alpha = self.softmax(att)  # (batch_size, seq_len)\n",
        "        context = (encoder_outputs * alpha.unsqueeze(2)).sum(dim=1)  # weighted sum -> (batch_size, feature_dim)\n",
    ...
    },
    {
      "cell_type": "code",
      "source": [
        "# Captioning model using GRU and additive attention over VAE-encoded image features\n",
        ...
        "        super().__init__()\n",
        "\n",
        "        self.code_seq_len = 64  # because 128x128 image and pooling factor 32 → 4x4 → 16x16 → 8x8 = 64\n",
        ...
        "\n",
        "        context, _ = self.attention(code_seq, h_t)\n",
        ...
        "           if words[-1] == self.stop_index and len(words) >= 5:\n",
        "            break\n",
        "\n",
        "           gru_input = torch.cat((w_t, context.unsqueeze(1)), dim=-1)\n",
